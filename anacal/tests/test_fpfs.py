...
def test_shear2():
    ...
    # test for g1
    shear1 = np.average(ells['fpfs_e1']) / np.average(ells['fpfs_R1E'])
    assert np.abs(shear1 - 0.) < thres
    # test for g2
    shear2 = np.average(ells['fpfs_e2']) / np.average(ells['fpfs_R2E'])
    assert np.abs(shear2 - shear_value) < thres
