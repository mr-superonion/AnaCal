"""Native coadd-PSF models: load LSST DM PSFs once (under the GIL),
evaluate them in pure C++ (GIL-free).

The C++ side (``anacal._anacal.psfmodel``) re-implements the DM chain
CoaddPsf -> WarpedPsf -> PsfexPsf; this module extracts the model
parameters from a DM ``CoaddPsf`` and fits the per-visit WCS mapping as
polynomials, which is the only approximation in the pipeline.  Loading
is the ONLY step that touches DM/Python objects.
