...
class sim_test:
    def __init__(
        self, shear, rng, scale=0.263, psf_fwhm=0.9, gal_hlr=0.5, ngrid=32
    ):
    ...
    def make_image(self, noise, psf_noise=0.0, do_shift=False):
        ...
        """
        if do_shift:
            dy, dx = self.rng.uniform(
                low=-self.scale / 2, high=self.scale / 2, size=2
            )
        ...
        if noise > 1e-10:
            ...
            img = self.img
        if psf_noise > 1e-10:
            psf = self.psf + self.rng.normal(
                scale=psf_noise, size=self.psf.shape
            )
        else:
            psf = self.psf
        return img, psf
...
class SimulationTask(object):
    def __init__(self, config_name):
        ...
        self.sim_method = cparser.get(
            "simulation", "sim_method", fallback="fft"
        )
        self.gal_type = cparser.get(
            "simulation", "gal_type", fallback="mixed"
        ).lower()
        ...
        self.do_shift = cparser.getboolean(
            "simulation", "do_shift", fallback=False
        )
        ...
        assert (
            self.image_ny == self.image_nx
        ), "'image_nx' must equals 'image_ny'!"
        self.psf_obj = None
        assert self.sim_method in ["fft", "mc"]
        assert self.gal_type in ["mixed", "sersic", "bulgedisk", "debug"]
        # PSF
        seeing = cparser.getfloat(
            "survey", "psf_fwhm", fallback=4.0 * self.scale
        )
        logging.info(
            "Using modelled Moffat PSF with seeing %.2f arcsec. " % seeing
        )
