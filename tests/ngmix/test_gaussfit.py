...
def test_ngmix_gaussian_fit1():
    ...
    img_array = (
        obj2.shift((0.5 - 0.1) * scale, (0.5 + 0.1) * scale)
        .drawImage(
            nx=nx,
            ny=ny,
            scale=scale,
    ...
    np.testing.assert_allclose(
        result[0].model.x1.v / scale - 32, -0.1, atol=1e-6
    )
    np.testing.assert_allclose(
        result[0].model.x2.v / scale - 32, 0.1, atol=1e-6
    )
    np.testing.assert_allclose(
        result[0].model.A.v,
        1.0,
...
def test_ngmix_gaussian_fit4():
    ...
    fitter = anacal.ngmix.GaussFit(scale=scale, sigma_arcsec=sigma_arcsec)
