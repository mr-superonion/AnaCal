...
def test_ngmix_gaussian():
    ...
    GaussModel.set_params([rho, g1, g2, x, y])

    m = GaussModel.apply(xc, yc)
    np.testing.assert_almost_equal(m.v.v, 0.9889981393251657)

    res = np.array([m.v_rho.v, m.v_g1.v, m.v_g2.v, m.v_x.v, m.v_y.v])
    res_target = np.array(
        [
            0.0182351950512148,
    ...
    res = np.array([m.v_rhorho.v, m.v_g1g1.v, m.v_g2g2.v, m.v_xx.v, m.v_yy.v])
    res_target = np.array(
        [
            -0.04525176622848741,
